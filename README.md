<div align="center">

![Top of pokeball](https://user-images.githubusercontent.com/44261381/209363264-ac854d3c-2cc2-44c4-928e-8a08d1013f46.png)

<details>
<summary>Open profile</summary>

[comment]: <> (View Counter)
<br>
<div>
  <div align=center>
      <img height="200" alt="Screenshot_2022-12-22_at_23 08 11-removebg-preview" src="https://i.ibb.co/cXH04KJ/Or-Asaf-wedding-333-of-1156.jpgc" alt="Avatar photo of William Guerrand">
  </div>
  <div align=center>
      <a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=VT323&size=35&duration=3500&pause=300&color=A89568&center=true&vCenter=true&width=500&lines=Hey%2C+I'm+Nitsan+Cohen;Welcome+to+my+profile!;Description+of+myself%3A;Frontend+Developer;Inquisitive+by+nature;AI+enthusiast;Animal+lover;Confident+and+ambitious;Long-distance+athlete;Young+at+heart;Always+cerious;Thrill+seeker" alt="Typing SVG" /></a>
  </div>
</div>

<details>
<summary>About me</summary>

<div align="left">

```ts
interface Nitsan {
    constructor(
        city: string = 'Harish, Israel',
        languages: string = 'English, Hebrew',
        jobTitle: string = 'Frontend Developer & Prompt Engineer',
        specialization: string = 'Building full-fledged web applications with React, Vue & AWS',
        expertise: string = 'Cloud architecture, Frontend development, DevOps & Prompt Engineering',
        aiExpertise: string = 'LLM prompt engineering, AI system design & optimization',
        interests: string = 'AI, writing & problem-solving',
        hobbies: string = 'Gardening, Animal care, Praying',
        education: string = 'Self :)',
        approachable: string = 'Yes, to collaborate on exciting projects, don\'t hesitate to reach out',
        strength: string = 'Problem solving, cloud architecture & AI systems',
        weakness: string = 'Not leaving a problem until it is solved',
        birthday: Date = new Date(1988, 11, 24)
    ): Nitsan;
}
```

</div>
</details>

<details>
<summary>Tools</summary>
<div>
  <p style="display: inline-block;" align="center">
    <kbd>
      <kbd>Programming Languages</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" /> 
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" /> 
    </kbd>
    <kbd>
      <kbd>Front-end & Frameworks</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" /> 
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-plain-wordmark.svg" /> 
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-plain.svg" /> 
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/gatsby/gatsby-original.svg" />
    </kbd>
    <kbd>
      <kbd>Back-end & Database</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-plain.svg" />
    </kbd>
    <br>
    <br>
    <kbd>
      <kbd>AI & ML Tools</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" />
      <img width="30px" src="https://upload.wikimedia.org/wikipedia/commons/0/04/ChatGPT_logo.svg" />
    </kbd>
    <kbd>
      <kbd>Cloud & DevOps</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-plain.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-plain.svg" />
    </kbd>
    <kbd>
      <kbd>Deployment & Hosting</kbd>
      <br>
      <br>
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/heroku/heroku-plain.svg" />
      <img width="30px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/netlify/netlify-original.svg" />
    </kbd>
  </p>
</div>
</details>

<details>
<summary>Words of Wisdom</summary>
<br>
One of my favorite Jewish teachings
<blockquote>
"Who is wise? One who learns from every person. Who is strong? One who overpowers their inclinations. Who is rich? One who is satisfied with their portion. Who is honorable? One who honors other people."
<br><strong>– Pirkei Avot (Ethics of the Fathers) 4:1</strong>
</blockquote>
</details>

<details>
<summary>Daily Inspiration</summary>
<br>
<small><i>A daily dose of Jewish wisdom to inspire growth and learning</i></small>
<br>
<div align="center">
<blockquote>
"In Jewish tradition, every day is an opportunity for growth, learning, and Tikkun Olam (repairing the world)."
</blockquote>
</div>
</details>

<details>
<summary>Free DOSE hit</summary>
<br>
<small><i>DOSE (dopamine, oxytocin, serotonin & endorphin), refresh page if dose was ineffective.</i></small>
<br>
<div align="center"><img src="https://readme-jokes.vercel.app/api?theme=monokai" alt="Jokes Card" /></div>
</details>

<details>
<summary>What can I do for you?</summary>
<table style="border: none">
<tr>
<td width="50%" valign="top">

## Let's Work on Your Project Together!

If you have any questions about web development and writing mistake-free documentation, feel free to <a href="mailto:nitsan@cohen.codes">contact me by email</a>, I won't bite, I promise.

</td>
<td width="50%" valign="top">

## It's not perfect, isn't it?

**<img alt="Feedback" src="https://img.shields.io/badge/Ask%20me-anything-1abc9c.svg">**

<blockquote>"I think it's very important to have a feedback loop, where you're constantly thinking about what you've done and how you could be doing it better."
<br><strong>– Elon Musk</strong></blockquote>

</td>
</tr>
</table>
</details>

</details>

![Bottom of pokeball](https://user-images.githubusercontent.com/44261381/209363271-905d2a5e-8a18-44c0-a450-45dddd4d5036.png)

</div>

<div align=center>
  <a href="https://www.linkedin.com/in/nitsan-cohen/"><img src="https://img.shields.io/static/v1?style=for-the-badge&message=LinkedIn&color=0A66C2&logo=LinkedIn&logoColor=FFFFFF&label=" alt="LinkedIn" /></a>
  <a href="mailto:nitsan@cohen.codes?subject=Hi%20Nitsan%20,%20nice%20to%20meet%20you!"><img alt="Email" src="https://img.shields.io/static/v1?style=for-the-badge&message=Gmail&color=EA4335&logo=Gmail&logoColor=FFFFFF&label=" /></a>
  <a href="https://cohen.codes/"><img src="https://img.shields.io/static/v1?style=for-the-badge&message=My+website&color=8e7e57&logo=data:image/png;base64,&label=" alt="My website" /></a>  
</div>
</div>

    
-----
<!-- Credits: [Austinae](https://github.com/Austinae)
Last Edited on: 23/12/2022 --!>
